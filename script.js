...
const calcMovementDate = function (date) {
...
const displayMovements = function (acc, sort = false) {
  ...
  const displayDate = new Intl.DateTimeFormat(acc.locale, options).format(now);
  ...
  movs.forEach(function (mov, i) {
    ...
    const displayDate = calcMovementDate(movDate);
...
const calcDisplaySummary = function (acc) {
  ...
  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = formatNum(Math.abs(out), acc.locale, acc.currency);
